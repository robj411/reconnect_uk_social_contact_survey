...
bootstrap_sample <- function(
  ...
  # data <- data %>% select(p_id, all_of(p_var), post_strat_weight, c_location, contains(tolower(locn)), starts_with('cag'))
  
  p_key <- "p_group"
  
  data <- data %>%
    mutate(
      # make a stable joint stratum label; choose a separator you’ll never see in values
      p_group = do.call(paste, c(across(all_of(p_var)), sep = "_"))
    ) %>%
    select(
      p_id,
      all_of(p_var),        # keep originals
      all_of(p_key),        # add key
      post_strat_weight,
      c_location,
      contains(locn, ignore.case = TRUE),
      starts_with("cag")
    )
  
  p_vec <- unique(data[[p_key]])
  
  # p_vec <- unname(unlist(unique(data %>% select(all_of(p_var)))))
  c_vec <- gsub('cag_','',unique(colnames(data %>% select(starts_with('cag')))))
  
  # main output
  bs_out <- CJ(c_location = locn, bs_index = 1:bs, p_key = p_vec, c_var = c_vec, mu = -8, k = -8)
  
  # cat(format_label(p_key), 'x', format_label(c_var), 'x', firstup(locn))
  ...
  for(p in p_vec){
    ...
    for(bs_i in 1:bs) {
      ...
      ids_df <- unique(data_imputed[get(p_key) == p, c('p_id', 'row_id', 'post_strat_weight')])
      row_samples_index <- sample(nrow(ids_df), replace = T, prob = ids_df$post_strat_weight)
...
nb_matrix_fit <- function(
  ...
  if(save == T){
    saveRDS(weighted_bs, here::here('results',paste0('nb_',paste0(gsub('p_','',participant_var),collapse='_'),'_', n_bootstrap,'.rds')))  
  }
...
normalise_matrix <- function(matrix,
  ...
  population_dt <- population_dt %>% select(!!sym(p_var_name), proportion)
  colnames(population_dt) <- c('p_key','proportion')
  
  scaled_matrix_1 <- matrix %>% arrange(bs_index, p_key) %>% 
    left_join(population_dt, by='p_key') %>% mutate(scaled_mu = mu*proportion) %>% 
    select(c_location, bs_index, p_key, c_var, mu, proportion, scaled_mu)
  
  transposed_scaled_matrix_1 <- scaled_matrix_1 %>% select('c_location', 'bs_index', 'p_key', 'c_var', 'scaled_mu')
  colnames(transposed_scaled_matrix_1) <- c('c_location', 'bs_index', 'c_var', 'p_key', 'scaled_mu_t')
  
  scaled_matrix_2 <- scaled_matrix_1 %>% left_join(transposed_scaled_matrix_1, by=c('c_location', 'bs_index', 'c_var', 'p_key')) %>% 
    mutate(mean_scaled_mu = (scaled_mu + scaled_mu_t)/2) %>% mutate(unscaled_mean_scaled_mu = mean_scaled_mu/proportion)
  
  matrix_out <- scaled_matrix_2 %>% select(c_location, bs_index, p_key, c_var, unscaled_mean_scaled_mu) %>% 
    setnames('unscaled_mean_scaled_mu', 'mu')
  
  if(per_capita == T){
    matrix_out %>% arrange(bs_index, p_key) %>% 
      left_join(population_dt %>% rename('c_var' = 'p_key'), by='c_var') %>% mutate(scaled_mu = mu/(proportion*sum(age_sex_strata$n))) %>% 
      select(c_location, bs_index, p_key, c_var, scaled_mu) %>% rename('mu' = 'scaled_mu')
  }else{
    matrix_out
  }
}
...
fcn_assortativity <- function(
  ...
  if('p_key' %notin% colnames(n)){
    p_var_col <- which(substr(colnames(n), 1, 2) == 'p_')
    setnames(n, colnames(n)[p_var_col], 'p_key')
  }
  
  n <- n %>% select('p_key','proportion')
  ...
  selection_vars <- c('p_key','c_var','mu')
  
  beta_thin <- beta %>% select(!!!syms(bs_vars), !!!syms(selection_vars))
  
  if(!is.null(bs_vars)){
    if(nrow(beta_thin) != n_distinct(beta_thin$bs_index)*n_distinct(beta_thin$p_key)*n_distinct(beta_thin$c_var)){
      stop('Contact matrix defined by index other than bs.')
    }
  }else{
    if(nrow(beta_thin) != n_distinct(beta_thin$p_key)*n_distinct(beta_thin$c_var)){
      stop('Contact matrix defined by index other than bs.')
    }
  }
  
  beta_hat <- beta_thin %>% left_join(n, by = 'p_key') %>% mutate(beta_hat_var = mu*proportion)
  
  beta_hat$p_key <- factor(beta_hat$p_key, levels = order)
  beta_hat$c_var <- factor(beta_hat$c_var, levels = order)
  
  B <- beta_hat %>% left_join(beta_hat %>% group_by(!!!syms(bs_vars), c_var) %>% summarise(sum_beta_hat_var = sum(beta_hat_var,na.rm = T)),
                              by = c(bs_vars,'c_var')) %>% 
    mutate(B_var = beta_hat_var/sum_beta_hat_var)
  
  # B %>% group_by(p_key, c_var) %>% summarise(mean_B = mean(B_var)) %>% 
  #   ggplot() + geom_tile(aes(x = p_key, y = c_var, fill = mean_B)) +
  #   theme_minimal() + geom_text(aes(x = p_key, y = c_var, label = round(mean_B, 2)), col = 'white') +
  #   labs(x = 'Participant', y = 'Contact', fill = 'Proportion of\ninfections') + theme(text = element_text(size = 14))
  
  Q <- B %>% filter(p_key == c_var) %>% group_by(!!!syms(bs_vars)) %>% 
    summarise(sum_Bii = sum(B_var,na.rm=T)) %>% mutate(Q_var = (sum_Bii - 1)/(n_distinct(beta_thin$p_key) - 1)) %>% 
    select(!!!syms(bs_vars), Q_var) %>% filter(!is.na(Q_var))
  ...
  if(!is.null(bs_vars)){
    ...
    for(bs in q$bs_index){
      ...
      matrix <- beta_hat %>% filter(bs_index == bs, !is.na(c_var), !is.na(p_key)) %>% arrange(p_key, c_var) %>% 
        select(p_key, c_var, mu) %>% pivot_wider(id_cols = p_key, names_from = c_var, values_from = mu) %>% 
        select(!p_key)
    ...
    matrix <- beta_hat %>% arrange(p_key, c_var) %>% 
      select(p_key, c_var, mu) %>% pivot_wider(id_cols = p_key, names_from = c_var, values_from = mu) %>% 
      select(!p_key)
  ...
  if(!is.null(bs_vars)){
    ...
    return(data.table(
      variable = c('Q','q'),
      mean = c(mean(Q$Q_var, na.rm=T), mean(q$q_var, na.rm=T)),
      lower_ci = c(quantile(Q$Q_var, (1 - ci)/2), quantile(q$q_var, (1 - ci)/2)),
      upper_ci = c(quantile(Q$Q_var, 1 - (1 - ci)/2), quantile(q$q_var, 1 - (1 - ci)/2))
    ))
...
plot_mu_matrix <- function(matrix_data,
  ...
  if(p_var_name == 'p_key'){
    matrix_data$p_var <- factor(matrix_data$p_key,
                                levels = unique(eth_age$p_key))
    matrix_data <- matrix_data[!is.na(matrix_data$p_var),]
  }
  if(c_var_name == 'c_var'){
    # browser()
    matrix_data$c_var <- factor(matrix_data$c_var,
                                levels = unique(eth_age$p_key))
    matrix_data <- matrix_data[!is.na(matrix_data$c_var),]
  }
  ...
  if(uncertainty == F){
    ...
    matrix_data %>% group_by(p_var, c_var) %>% 
      summarise(width = quantile(!!sym(variable), 0.975, na.rm=T) - quantile(!!sym(variable), 0.025, na.rm=T)) %>% 
      ggplot() + 
